...
class PharmacyService {
  ...
  /**
   * Update medication request status (for debug/simulation purposes)
   */
  async updateRequestStatus(id: number, status: string): Promise<MedicationRequest> {
    try {
      const response = await api.patch(`${PHARMACY_BASE_URL}/requests/${id}/`, {
        status: status
      });

      // Fetch inventory for mapping
      const inventory = await this.getInventory();
      const req = response.data;
      ...
      const errorMessage = error.response?.data?.error
        || error.response?.data?.message
        || error.response?.data
        || 'Failed to update medication request status';
